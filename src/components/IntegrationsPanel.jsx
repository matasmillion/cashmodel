...
// Build a lookup of balance by account mask from lastSync data.
// Returns { [mask]: { balance, isCredit } }
function buildBalanceLookup(lastSync) {
  if (!lastSync) return {};
  const map = {};
  for (const a of (lastSync.depositoryAccounts || [])) {
    if (a.mask) map[a.mask] = { balance: a.balance, isCredit: false };
  }
  for (const a of (lastSync.creditAccounts || [])) {
    if (a.mask) map[a.mask] = { balance: a.balance, isCredit: true };
  }
  return map;
}
...
function PlaidCard({ dispatch }) {
  ...
  // Auto-load cached balances on mount so account rows show numbers immediately.
  useEffect(() => {
    if (loading) return;
    if (items.length === 0) return;
    syncPlaidActuals({ realTime: false }).then(({ totals, depositoryAccounts, creditAccounts }) => {
  ...
  async function handleSync() {
    ...
    try {
      ...
      // Push depository cash into the seed.
              ...
              <div key={item.item_id} className="p-2 rounded-lg text-xs" style={{ background: 'white', border: `1px solid ${FR.sand}` }}>
                ...
                <div className="space-y-0.5" style={{ color: FR.stone }}>
                  {(item.accounts || []).map((a, i) => {
                    const balInfo = a.mask ? buildBalanceLookup(lastSync)[a.mask] : null;
                    const isCredit = balInfo?.isCredit ?? a.type === 'credit';
                    const bal = balInfo?.balance ?? null;
                    const displayBal = bal != null
                      ? (isCredit ? `-$${bal.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : `$${bal.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`)
                      : (a.subtype || a.type);
                    return (
                      <div key={i} className="flex justify-between">
                        <span>{a.name}{a.mask ? ` ••${a.mask}` : ''}</span>
                        <span className="text-[10px]" style={{ color: bal != null ? (isCredit ? '#A32D2D' : FR.slate) : FR.stone, fontVariantNumeric: 'tabular-nums' }}>
                          {displayBal}
                        </span>
                      </div>
                    );
